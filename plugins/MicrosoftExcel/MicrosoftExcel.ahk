MicrosoftExcel:
    ...
    vim.comment("<Select_Mode_MicrosoftExcel>", "Select模式")
    vim.comment("<Appskey>", "右键")
    vim.comment("<vselection>", "按v选择")
    ...
    vim.comment("<open_path>", "打开当前工作簿所在路径")
    ...
    ;Select模式及快捷键
    vim.mode("select", "MicrosoftExcel")
    vim.map("<esc>", "<Normal_Mode_MicrosoftExcel>", "MicrosoftExcel")
    ...
    vim.map("gg", "<MicrosoftExcel_向上选择到开头>", "MicrosoftExcel")
    vim.map("G", "<MicrosoftExcel_向上选择到结尾>", "MicrosoftExcel")
    ...
    vim.map("v", "<Select_Mode_MicrosoftExcel>", "MicrosoftExcel")
    ...
    ;o插入/O插入在右
    vim.map("op", "<open_path>", "MicrosoftExcel")
    ...
    ;右键
    vim.map("oo", "<AppsKey>", "MicrosoftExcel")
    ...
    ;vim.map("<c-d>", "<MicrosoftExcel_PageDown>", "MicrosoftExcel")
    ;vim.map("<c-u>", "<MicrosoftExcel_PageUp>", "MicrosoftExcel")
    ...
    vim.map("yr", "<MicrosoftExcel_Copy_Row_复制整行>", "MicrosoftExcel")
    vim.map("yc", "<MicrosoftExcel_Copy_Col_复制整列>", "MicrosoftExcel")
...
<open_path>:
{
    ;~可能不同的excel版本,不同的win版本, 会不同吧. 我是用便携版测试的....也考虑了"我的文档"的路径情况.如果你xls文档是放在其他"我的xx"目录,暂未考虑

    ;~自定义tc路径
    tc := "C:\TOTALCMD\Totalcmd.exe"
    WinGet, var, ProcessName, A
    IfNotInString,var,excel
        return
    Send {F12}
    WinWait, 另存为
    ControlGetText, TW, ToolbarWindow324, 另存为
    address := StrReplace(TW,"地址: ","")
    ControlGetText, file, Edit1, 另存为
    initial_ad:=SubStr(address,2,2) ; this part is to determine if it's in the Documents folder which typically doesn't include C:\ at the beginning of the file location in the Save As window
    If (initial_ad != ":\")
        address:="C:\Users\" . A_UserName . "\" . address
    FullFilePath=%address%\%file%
    StringReplace,FULLFILEPATH,FULLFILEPATH,库\文档,Documents
    clipboard:=FullFilePath
    WinClose, 另存为
    Sleep 50
    ;MsgBox % clipboard
    run %tc% /O /T /A /S /L="%Clipboard%"
    return
}
