    ...
    vim.Comment("<TC_FirstFile>", "跳到指定文件夹的第一个文件")
    ...
    vim.map("f1", "<TC_第一个文件>", "TTOTAL_CMD")
    ...
    vim.map(";m", "<cm_ShowQuickSearch>", "TTOTAL_CMD")
    vim.map("/", "<cm_ShowQuickSearch>", "TTOTAL_CMD")
    vim.map("<shift>", "<cm_ShowQuickSearch>", "TTOTAL_CMD")
...
TCCOMMAND:
    ...
    vim.Comment("<cm_MatchSrc>", "目标 = 来源(当前窗口复制到对面窗口去)")
...
<cm_ahk>:
    ...
    If !ErrorLevel
    {
        ...
        IF ext in bmp,jpg,png ;用ps 编辑 
            run,"C:\Adobe Photoshop CS5 Extended\Adobe Photoshop CS5\Photoshop.exe" "%FullFileName%"
        else if ext in txt,c,ini,vimrc ;用vim 编辑 
            Run "C:\Users\Administrator\AppData\Local\Programs\Microsoft VS Code\Code.exe"
        else if ext in php,html,ahk ; 用phpstorm 编辑 
            run,"C:\Program Files\JetBrains\PhpStorm 2018.2.4\bin\phpstorm64.exe" "%FullFileName%"
        else
            MsgBox 没有对应扩展名的程序，请先配置
    }
Clipboard = %temp%
Return
return
