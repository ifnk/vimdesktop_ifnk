...
Class __vim
{
    ...
    Convert2VIM(key)
    {
        ...
        if RegExMatch(key, "i)^((F1)|(F2)|(F3)|(F4)|(F5)|(F6)|(F7)|(F8)|(F9)|(F10)|(F11)|(F12))$")
            return "<" key ">"
        if RegExMatch(key, "i)^((AppsKey)|(Tab)|(Enter)|(Space)|(Home)|(End)|(CapsLock)|(ScrollLock)|(Up)|(Down)|(Left)|(Right)|(PgUp)|(PgDn)|(Pause)|(Numpad9))$")
            return "<" key ">"
    ...
    Convert2AHK(key, ToSend=false)
    {
        ...
        if RegExMatch(key, "^<.*>$")
        {
            ...
            if RegExMatch(key, "i)^((AppsKey)|(Tab)|(Enter)|(Space)|(Home)|(End)|(CapsLock)|(ScrollLock)|(Up)|(Down)|(Left)|(Right)|(PgUp)|(PgDn)|(BS)|(ESC)|(Insert)|(Delete)|(Pause)|(Numpad9))$")
                return ToSend ? "{" key "}" : key
